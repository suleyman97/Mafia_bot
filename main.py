...
HELP_GROUP = (
    "Команды (для админов):\n"
    "• Ответом на пост записи: /track — начать отслеживать этот пост\n"
    "• /track notify — начать и сразу разослать по текущему списку\n"
...
def is_authorized_source(message: Message, admin_ids: set[int]) -> bool:
    """Allow organizer/admin messages, including 'anonymous admin' posts.

    Cases:
    - normal admin: from_user.id in ADMIN_IDS
    - anonymous admin / 'send as chat' / 'send as channel': sender_chat is set
      (only admins can do this in a group), so we treat it as authorized.
    """
    if message.from_user is not None and message.from_user.id in admin_ids:
        return True
...
async def cmd_players(message: Message, service: BotService):
    ...
    if not players:
        await message.answer("Пока нет игроков в базе. Они появятся после /track и обновлений записи.")
        return
...
async def auto_track_post(
    ...
    """
    if not is_authorized_source(message, admin_ids):
        return
...
async def cmd_track(message: Message, command: CommandObject, service: BotService, bot: Bot, admin_ids: set[int]):
    if message.chat.type not in (ChatType.GROUP, ChatType.SUPERGROUP):
        await message.answer("Эта команда работает только в группе.")
        return
    if not is_authorized_source(message, admin_ids):
        await message.reply("Только админы могут включать отслеживание.")
        return
    if not message.reply_to_message:
        await message.reply("Нужно ответить командой /track на пост записи организатора.")
        return

    tracked = message.reply_to_message
    text = tracked.text or tracked.caption or ""
    if not text:
        await message.reply("В посте нет текста для парсинга.")
        return

    notify_existing = bool(command.args and "notify" in command.args.lower())

    await service.set_tracked_message(message.chat.id, tracked.message_id)
    players_total, _, title = await service.update_from_post_and_notify(
        bot=bot,
        chat_id=message.chat.id,
    ...
    extra = " (и разослал текущий список)" if notify_existing else ""
    await message.reply(
        f"✅ Отслеживаю этот пост{extra}.\n"
        f"Название: <b>{html_escape(title)}</b>\n"
        f"Игроков в списке сейчас: {players_total}",
        parse_mode="HTML",
    )

@router.message(Command("untrack"))
async def cmd_untrack(message: Message, service: BotService, admin_ids: set[int]):
    if message.chat.type not in (ChatType.GROUP, ChatType.SUPERGROUP):
        await message.answer("Эта команда работает только в группе.")
        return
    if not is_authorized_source(message, admin_ids):
...
async def cmd_event(message: Message, service: BotService):
    ...
    if not mid:
        await message.reply("Сейчас ничего не отслеживается. Ответь на пост записи командой /track.")
        return
...
async def on_edited_message(message: Message, service: BotService, bot: Bot, admin_ids: set[int]):
    if message.chat.type not in (ChatType.GROUP, ChatType.SUPERGROUP):
        return
    ...
    if not tracked_id:
        if is_authorized_source(message, admin_ids) and is_candidate_event_post(text):
            await service.set_tracked_message(message.chat.id, message.message_id)
            players_total, title = await service.sync_event_from_post(message.chat.id, message.message_id, text)
            log.info("Auto-tracking enabled (via edit): chat=%s message=%s title=%r players=%s", message.chat.id, message.message_id, title, players_total)
        return
